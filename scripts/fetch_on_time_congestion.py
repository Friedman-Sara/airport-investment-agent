...
def build_airport_summary(airport_code: str, metrics: dict) -> dict:
    ...
    return {
        ...
        "delay_eligible_flights": decimal_to_number(
        ...
        "departure_delay_observations": decimal_to_number(
            metrics["departure_delay_observations"]
        ),
        ...
        "taxi_out_observations": decimal_to_number(
            metrics["taxi_out_observations"]
        ),
...
def fetch_and_process() -> dict:
    ...
    return build_result(
        airport_summaries,
        source_urls,
        "Official public monthly bulk-download endpoint",
    )


def process_existing_raw() -> dict:
    if not RAW_OUTPUT.exists():
        raise FileNotFoundError(f"Filtered raw file not found: {RAW_OUTPUT}")

    print(f"Processing existing filtered raw data: {RAW_OUTPUT}...")
    metrics_by_airport = {
        airport_code: empty_metrics() for airport_code in AIRPORT_CODES
    }

    with RAW_OUTPUT.open("r", encoding="utf-8-sig", newline="") as raw_file:
        reader = csv.DictReader(raw_file)
        validate_header(reader.fieldnames, str(RAW_OUTPUT))

        for row in reader:
            airport_code = row["Origin"].strip().upper()
            if airport_code not in AIRPORT_CODES:
                continue
            process_row(row, metrics_by_airport[airport_code])
    ...
    source_urls = [
        BASE_URL.format(year=year, month=month) for year, month in MONTHS
    ]

    return build_result(
        airport_summaries,
        source_urls,
        "Cached filtered raw data from official monthly bulk downloads",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--from-raw",
        action="store_true",
        help="Recalculate from the existing filtered raw CSV without downloading",
    )
    arguments = parser.parse_args()

    result = process_existing_raw() if arguments.from_raw else fetch_and_process()
